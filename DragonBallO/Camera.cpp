...
//Restrict NPC movement to camera view
void Camera::RestrictTargetToCamera(Entity &target) {
	...
	if (targetBound.left < mPos.x) {
		target.mPos.x = -(float)target.mTopLeftCollOffset.x;
		target.mPushbackSides |= 1 << 0;	//can't pushback from right...
	}	//collides right...
	else if (targetBound.right > mPos.x + SCREEN_WIDTH) {
		target.mPos.x = float(mPos.x + SCREEN_WIDTH - target.mSize.x + target.mBottomRightCollOffset.x);
		target.mPushbackSides |= 1 << 2;	//can't pushback from left...
	}

	//collides top...
	if (targetBound.top < mPos.y) {
		target.mPos.y = -(float)target.mTopLeftCollOffset.y;
		target.mPushbackSides |= 1 << 3;	//can't pushback from bottom...
	}	//collides bottom...
	else if (targetBound.bottom > mPos.y + SCREEN_HEIGHT) {
		target.mPos.y = float(mPos.y + SCREEN_HEIGHT - target.mSize.y + target.mBottomRightCollOffset.y);
